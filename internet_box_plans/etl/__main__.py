"""main module for internet box plans ETL"""
...
from etl.transform.internet_box_plans_transformation import InternetBoxPlansTransformer
...
def extract(
    ...
    """ETL extract command to scrape internet box plans for a given prospect
      profile scenario.
    Args:
    ...
    logger.info("Data loader initialized: %s", data_loader)
    browser = DynamicSearchBrowser(
        obj_actions,
        base_url=BASE_URL,
        data_loader=data_loader,
    )
    logger.info("Browser initialized: %s", browser)
    browser.run()
...
def transform(
    ...
    """Transform step of the ETL pipeline scraping internet box plans
...
def load(
    ...
    """Load step of the ETL pipeline scraping internet box plans
